...
After cloning, the repo has source code + `generated_pairs_openai.csv` (the LLM-generated data). CultureBank is loaded from HuggingFace at runtime — no manual download needed.
...
# 2. Assemble train/val/test splits from the tracked generated_pairs_openai.csv
...
To **regenerate** `generated_pairs_openai.csv` from scratch (~5 min, resumable):
```bash
...
├── prepare_dataset_v2.py          # Assemble and split dataset from generated_pairs_openai.csv
...
├── generated_pairs_openai.csv            # LLM output (tracked): culture, original_norm, norm, generic
...
Only source code (`.py`), `generated_pairs_openai.csv`, the PDF spec, `requirements.txt`, and `README.md` are tracked. Everything else is generated or secret:

| Pattern | What it excludes |
|---------|-----------------|
| `.env` | API keys |
| `norm_classifier_bert/` | Saved BERT model weights (~440 MB) |
| `*.csv` (except `generated_pairs_openai.csv`) | train/val/test splits |
